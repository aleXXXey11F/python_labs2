...
from src.lab03.models import CityBus, TouristBus, SchoolBus
...
def scenario_7_filtering_by_type():
    ...
    if total_capacity_city > 0:
        avg_load_city = sum(bus.current_passengers for bus in city_fleet) / total_capacity_city * 100
        print(f"  Городские: общая вместимость {total_capacity_city}, средняя загрузка: {avg_load_city:.1f}%")
    if total_capacity_tourist > 0:
        avg_load_tourist = sum(bus.current_passengers for bus in tourist_fleet) / total_capacity_tourist * 100
        print(f"  Туристические: общая вместимость {total_capacity_tourist}, средняя загрузка: {avg_load_tourist:.1f}%")
    if total_capacity_school > 0:
        avg_load_school = sum(bus.current_passengers for bus in school_fleet) / total_capacity_school * 100
        print(f"  Школьные: общая вместимость {total_capacity_school}, средняя загрузка: {avg_load_school:.1f}%")
