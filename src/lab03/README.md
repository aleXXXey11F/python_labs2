...
Краткая суть: на основе базового класса Bus создана иерархия из трёх типов автобусов (городской, междугородний, электробус), где каждый дочерний класс добавляет свои атрибуты и по-своему реализует общие методы (расчёт стоимости, отображение информации). Коллекция Fleet работает со всеми типами через единый интерфейс — без проверок if type ==, только через полиморфные вызовы.
